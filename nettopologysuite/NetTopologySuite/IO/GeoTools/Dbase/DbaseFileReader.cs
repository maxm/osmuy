					...
					{                
						...
						switch (tempFieldType)
						{
							...
							case 'C':   // character record.
								var sbuffer = new byte[tempFieldLength];
								sbuffer = _dbfStream.ReadBytes(tempFieldLength);
								...
								tempObject = Encoding.UTF8.GetString(sbuffer).Trim().Replace("\0",String.Empty);   //.ToCharArray();
								break;
